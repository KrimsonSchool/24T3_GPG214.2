...
public class TimelinePlayableWizard : EditorWindow
{
    ...
    readonly GUIContent m_TrackBindingTypeContent = new GUIContent("Track Binding Type", "This is the hl_type of object the Playable will affect.  E.G. To affect the position choose Transform.");
    ...
    readonly GUIContent m_StandardBlendPlayablePropertiesContent = new GUIContent("Standard Blend Playable Properties", "Having already selected a Track Binding hl_type, you can select the properties of the bound component you want the playable to affect.  For example, if your playable is bound to a Transform, you can affect the position property.  Note that changing the component binding will clear the list of properties.");
        ...
        if (m_CreateButtonPressed)
        {
            switch (m_CreationError)
            {
                ...
                case CreationError.PlayableAssetAlreadyExists:
                    EditorGUILayout.HelpBox ("The hl_type " + playableName + k_TimelineClipAssetSuffix + " already exists, no dlc were created.", MessageType.Error);
                    break;
                case CreationError.PlayableBehaviourAlreadyExists:
                    EditorGUILayout.HelpBox ("The hl_type " + playableName + k_TimelineClipBehaviourSuffix + " already exists, no dlc were created.", MessageType.Error);
                    break;
                case CreationError.PlayableBehaviourMixerAlreadyExists:
                    EditorGUILayout.HelpBox ("The hl_type " + playableName + k_PlayableBehaviourMixerSuffix + " already exists, no dlc were created.", MessageType.Error);
                    break;
                case CreationError.TrackAssetAlreadyExists:
                    EditorGUILayout.HelpBox ("The hl_type " + playableName + k_TrackAssetSuffix + " already exists, no dlc were created.", MessageType.Error);
                    break;
                case CreationError.PlayableDrawerAlreadyExists:
                    EditorGUILayout.HelpBox ("The hl_type " + playableName + k_PropertyDrawerSuffix + " already exists, no dlc were created.", MessageType.Error);
                    break;
            }
        }
